...
class CuteInterpreter(object):
    ...
    def run_arith(self, arith_node):
        ...
        if arith_node.type is TokenType.PLUS:
            ...
            if (int(rhs1.value) is int(rhs2.value)):
                return self.TRUE_NODE
            else:
                return self.FALSE_NODE
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            #작성
            #rhs2는 무조건 list라고 가정
            if expr_rhs2.value is None:
                expr_rhs1.next = expr_rhs2.value
                expr_rhs2.value = expr_rhs1
                return create_quote_node(expr_rhs2)
            if is_quote_list(expr_rhs1) is not True and is_quote_list(expr_rhs2) is True:
                expr_rhs1.next = expr_rhs2.value.next.value
                expr_rhs2.value.next.value = expr_rhs1
                return expr_rhs2
            elif is_quote_list(expr_rhs1) is True and is_quote_list(expr_rhs2) is True:
                expr_rhs1.value.next.next = expr_rhs2.value.next.value
                expr_rhs2.value.next.value = expr_rhs1.value.next
                return expr_rhs2
            ...
            if rhs1.type is TokenType.FALSE: return self.TRUE_NODE
            return self.FALSE_NODE
